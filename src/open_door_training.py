...
class goIFOhandle(smach.State):
    ...
    def execute(self, userata):
        ...
        while answer != 1 : 
            rospy.sleep (1)
            answer = self.door_info.call("arrive_at_destination",0).output_int
...
class getSOD(smach.State):
    ...
    def execute(self, userdata):
        ...
        #kdl_quaternion_face = frameIFOdoor_map_face.frame.M.GetQuaternion()
        #pose_stamped_IFOdoor_face = create_pose_stamped(kdl_vector_face[0], kdl_vector_face[1], kdl_vector_face[2], kdl_quaternion_face[0], kdl_quaternion_face[1], kdl_quaternion_face[2], kdl_quaternion_face[3])
...
class goAwayFromObstacle(smach.State):
    ...
    def execute (self, userdata):
        ...
        while dget < dmin:
            ...
            self.robot.base.force_drive(x_direction, y_direction, 0)
...
def sm_open_door_close(robot, arm, my_door):
    ...
    with sm_open_door_close:
        smach.StateMachine.add('goIFOhandle', goIFOhandle(my_door, robot), transitions={'IFO_handle' : 'updateHandleLocation', 'fail' : 'fail'})
...
def sm_cross_door(robot, arm, my_door):
    ...
    with sm_cross_door:
        smach.StateMachine.add('getSOD', getSOD(robot, my_door), transitions={'find_SOD' : 'goIFOdoor', 'fail' : 'fail'})
        smach.StateMachine.add('goIFOdoor', goIFOdoor(my_door, robot), transitions={'IFO_door' : 'update_door_state', 'fail' : 'fail'})
